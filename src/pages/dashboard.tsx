...
import { getPayrolls } from 'apiSdk/payrolls';
...
import { VacationListPage as ShowCaseList } from 'pages/vacations';
...
function DashboardPage() {
  ...
  const payrollStat = useSWR<PaginatedInterface<PayrollInterface>>(
    () => `/payrolls`,
    () => getPayrolls({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Payrolls',
      result: payrollStat?.data?.totalCount,
      isLoading: payrollStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`vacations`} passHref legacyBehavior>
                  <Button
